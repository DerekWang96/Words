...
public class ReviewActivity extends Activity implements View.OnClickListener{
    ...
    public void initDatas() throws ParseException {
        ...
        wordlist = new ACID(context,username).reviewword();//添加参数：读取数量
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_memorize_yes:
            {
                ...
                if (memoryCount < wordlist.size()) {
                    ...
                    ACID newACID = new ACID(context,username);
                    newACID.addintomywords(wordlist,wordbookTitle);
                }
...
//            progress = (progress+1)%wordlist.size();
    ...
    public void setUsername(){
        newfile nf =  new newfile(ReviewActivity.this);
        try {
            username = nf.readcurrentuser();
